    ...
    private static string CreateSBDMstrTbl(string RoleID, DataSet Ds, string tblname, string cls, string IsNewAdditionAllowed)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                sb.Append("<tr SBD='" + dt.Rows[i]["SBDID"] + "' SBDName='" + dt.Rows[i]["SBDName"] + "' FromDate='" + dt.Rows[i]["FromDate"] + "' ToDate='" + dt.Rows[i]["ToDate"] + "' strloc='" + sbLocation.ToString() + "' strchannel='" + sbChannel.ToString() + "' strSBD='" + sbBaseSBF.ToString() + "' Frequency='" + dt.Rows[i]["Frequency"].ToString() + "' flgEdit='0' iden='SBD' style='display: table-row;'>");
