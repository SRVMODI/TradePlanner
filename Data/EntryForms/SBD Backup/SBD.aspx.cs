...
public partial class _SBDMstr : System.Web.UI.Page
{
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (dt.Rows[i]["flgBookmark"].ToString() == "1")
                    sb.Append("<td iden='SBD'><img src='../../Images/bookmark.png' title='Active Bookmark' flgBookmark='1' onclick='fnManageBookMark(this);'/></td>");
                else
                    sb.Append("<td iden='SBD'><img src='../../Images/bookmark-inactive.png' title='InActive Bookmark' flgBookmark='0' onclick='fnManageBookMark(this);'/></td>");
        ...
        if (dr.Length > 0)
        {
            ...
            switch (flg)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    DataTable dtDistinctBaseSBF = dttemp.DefaultView.ToTable(true, "BaseNodeID");
                    for (int i = 0; i < dtDistinctBaseSBF.Rows.Count; i++)
                    {
                        DataTable dtProxySBF = dttemp.Select("BaseNodeID=" + dtDistinctBaseSBF.Rows[i]["BaseNodeID"].ToString()).CopyToDataTable();
                        sbDescr.Append(", " + dtProxySBF.Rows[0]["BaseSBF"].ToString());
                        sb.Append("*" + dtProxySBF.Rows[0]["BaseNodeID"].ToString() + "|" + dtProxySBF.Rows[0]["BaseNodeType"].ToString() + "|" + dtProxySBF.Rows.Count + "|" + dtProxySBF.Rows[0]["MoQ"].ToString());
                        
                        //sb.Append("|");
                        //DataTable dtDistinctProxyBrand = dtProxySBF.DefaultView.ToTable(true, "BrandNodeID");
                        //for (int j = 0; j < dtDistinctProxyBrand.Rows.Count; j++)
                        //{
                        //    if (j == 0)
                        //        sb.Append(dtDistinctProxyBrand.Rows[j]["BrandNodeID"].ToString());
                        //    else
                        //        sb.Append("^" + dtDistinctProxyBrand.Rows[j]["BrandNodeID"].ToString());
                        //}
                        //dtDistinctProxyBrand.Dispose();

                        sb.Append("-");
                        for (int j = 0; j < dtProxySBF.Rows.Count; j++)
                        {
                            if (j == 0)
                                sb.Append(dtProxySBF.Rows[j]["ProxyNodeID"].ToString() + "|" + dtProxySBF.Rows[j]["ProxyNodeType"].ToString());
                            else
                                sb.Append("^" + dtProxySBF.Rows[j]["ProxyNodeID"].ToString() + "|" + dtProxySBF.Rows[j]["ProxyNodeType"].ToString());
                        }
                        dtProxySBF.Dispose();
                    }
                    dtDistinctBaseSBF.Dispose();
                    sb.Append("||||" + strBrand);

                    //DataTable dtDistinctBrand = dttemp.DefaultView.ToTable(true, "BrandNodeID");
                    //for (int j = 0; j < dtDistinctBrand.Rows.Count; j++)
                    //{
                    //    if (j == 0)
                    //        sb.Append(dtDistinctBrand.Rows[j]["BrandNodeID"].ToString());
                    //    else
                    //        sb.Append("^" + dtDistinctBrand.Rows[j]["BrandNodeID"].ToString());
                    //}
                    //dtDistinctBrand.Dispose();
                    break;
            }
            return sbDescr.ToString().Substring(2) + "~" + sb.ToString().Substring(1) + "~0";
        }
        else
        {
            return "~~0";
    ...
    [System.Web.Services.WebMethod()]
    public static string fnSave(string SBDID, string SBDName, string FromDATE, string EndDate, string strChannel, string strLocation, string strBaseProduct, string UserID, string LoginID, string RoleID, string QtrNo, string QtrYear, object objBuckets, object objBucketValues, object objSBF, string flgSaveDuplicate, string Frequency, string StrBrand)
    {
        try
        {
            ...
            DataTable tblSBF = new DataTable();
            ...
            tblSBF = JsonConvert.DeserializeObject<DataTable>(strSBF);
            ...
            Scmd.CommandType = CommandType.StoredProcedure;
            Scmd.Parameters.AddWithValue("@SBDID", SBDID);
            Scmd.Parameters.AddWithValue("@SBDName", SBDName);
            Scmd.Parameters.AddWithValue("@FromDATE", FromDATE);
            Scmd.Parameters.AddWithValue("@EndDate", EndDate);
            Scmd.Parameters.AddWithValue("@strChannel", strChannel);
            Scmd.Parameters.AddWithValue("@strLocation", strLocation);
            Scmd.Parameters.AddWithValue("@strBaseProduct", strBaseProduct);
            Scmd.Parameters.AddWithValue("@UserID", UserID);
            Scmd.Parameters.AddWithValue("@LoginID", LoginID);
            Scmd.Parameters.AddWithValue("@RoleID", RoleID);
            Scmd.Parameters.AddWithValue("@QtrNo", QtrNo);
    ...
    [System.Web.Services.WebMethod()]
    public static string fnDelete(string SBDID)
    {
        try
        {
            SqlConnection Scon = new SqlConnection(ConfigurationManager.ConnectionStrings["strConn"].ConnectionString);
